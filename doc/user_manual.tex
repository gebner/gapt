...
\item iProver (\url{http://www.cs.man.ac.uk/~korovink/iprover/})
...
Some(p(0)-,
∀x (p(x) ⊃ p(s(x))) +^{0} (p(0)+ ⊃ p(s(0))-) +^{s(0)} (p(s(0))+ ⊃ p(s(s(0)))-)
:-
p(s(s(0)))+)
...
the tree grammar-based method described in~\cite{Eberhard15Inductive}. As of version 2.7 of GAPT,
...
[p144] ∀y (plus(#c(Z: Nat), y:Nat): Nat) = y,
∀y0 ¬#c(Z: Nat) = S(y0:Nat),
∀x0 (p(S(x0)): Nat) = x0,
∀z (minus(S(z), #c(Z: Nat)): Nat) = S(z),
∀y minus(#c(Z: Nat), y) = #c(Z: Nat),
∀z ∀x2 minus(S(z), S(x2)) = minus(z, x2),
∀z ∀y plus(S(z), y) = S(plus(z, y))
⊢
∀n ∀m minus(n, plus(n, m)) = #c(Z: Nat)    (CutRule(p26, Suc(0), p143, Ant(4)))
[p143] ∀y (plus(#c(Z: Nat), y:Nat): Nat) = y,
∀y0 ¬#c(Z: Nat) = S(y0:Nat),
∀x0 (p(S(x0)): Nat) = x0,
∀z (minus(S(z), #c(Z: Nat)): Nat) = S(z),
∀m
  ((⊤ ⊃ minus(#c(Z: Nat), plus(#c(Z: Nat), m)) = #c(Z: Nat)) ∧
      ∀n_0
...
\subsection{Induction-elimination}

As an extension of Gentzen cut-elimination, GAPT can also eliminate induction
inferences in a restricted class of proofs with quantifier-free conclusions.
...
res39: at.logic.gapt.proofs.expansion.ExpansionProofWithCut =
...
res40: at.logic.gapt.proofs.Sequent[at.logic.gapt.expr.Formula] = ∀x ∀y (P(x, y) ⊃ Q(x, y)) ⊢ ∃x ∃y (¬Q(x, y) ⊃ ¬P(x, y))

gapt> expansion.deep
res41: at.logic.gapt.proofs.Sequent[at.logic.gapt.expr.Formula] = ¬P(x, a) ∨ Q(x, a) ⊃ ¬P(b, y) ∨ Q(b, y), P(x, a) ⊃ Q(x, a) ⊢ ¬Q(b, y) ⊃ ¬P(b, y)
...
res43: at.logic.gapt.proofs.expansion.ExpansionProof =
∀x ∀y (P(x, y) ⊃ Q(x, y))
...
res44: at.logic.gapt.proofs.lk.LKProof =
...
res52: at.logic.gapt.grammars.VTRATG =
Non-terminal vectors: (x_0), (x_1), (x_2)
Terminals: 0, s:i>i
...
res54: Option[at.logic.gapt.proofs.expansion.ExpansionProof] =
Some(
:-
P(nil:list)- ∧
    (∀x ∀y ∀z (P(x) ⊃ P(cons(y:nat, cons(z:nat, x): list)))
...
res55: at.logic.gapt.proofs.lk.LKProof =
...
res56: at.logic.gapt.proofs.nd.NDProof =
...
The variable $y$ must not occur free in $\Gamma$ in case of $\forall$ introduction, and must not occur free in $\Pi$ or $B$ in case of $\exists$ elimination.
