...
class ResolutionTest extends SpecificationWithJUnit {
  ...
  "Paramodulation rule in Robinson Resolution" should {
    ...
    "correctly keep the context of demodulated formulas " in {
      ...
      val List(e1,e2,e3,p,q) = List(Equation(a,b), Equation(c,d), Equation(e,f), FOLAtom(P,a::Nil), FOLAtom(P,b::Nil)  )
...
/* using deprecated data structure. Please update.
  "Andrews Resolution" should {
    ...
    "handle strong quantifiers correctly" in {
      ...
      val z = Var("Z", i -> o )
      val args = x::y::z::Nil
      val tp = FOLFunctionType(To, args.map(a => a.exptype))
      ...
      val tp1 = FOLFunctionType(To, skt1.exptype::y.exptype::z.exptype::Nil)
      val tp2 = FOLFunctionType(To, skt2.exptype::y.exptype::z.exptype::Nil)
    ...
    "handle weak quantifiers and substitution correctly" in {
      val x = Var("X", i -> o )
      val f = Const("f", (i -> o) -> i )
      val xfx = App(x, App( f, x ) ).asInstanceOf[HOLFormula]
