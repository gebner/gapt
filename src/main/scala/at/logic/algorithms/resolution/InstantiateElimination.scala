...
object InstantiateElimination {
  ...
  def find_matching[A, B]( objects: List[A], targets: List[B], matches: ( A, B ) => Boolean ): Map[A, B] = NameReplacement.find_matching( objects, targets, matches )
  ...
  def extend_pmap( o: OccMap, f: VarSet, p: RobinsonResolutionProof, pmap: ProofMap ): ( ProofMap, OccMap, VarSet, RobinsonResolutionProof ) = {
