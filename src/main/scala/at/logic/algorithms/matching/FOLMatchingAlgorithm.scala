...
object FOLMatchingAlgorithm {
  ...
  def matchTerm( term1: FOLExpression, term2: FOLExpression, restrictedDomain: List[Var] ): Option[Substitution] =
    matchSetOfTuples( restrictedDomain, ( term1, term2 ) :: Nil, Nil ) match {
      case Some( ( Nil, ls ) ) => Some( Substitution( ls.map( x => ( x._1.asInstanceOf[Var], x._2 ) ) ) )
      case _                   => None
    }

  def restrictSubstitution( delvars: List[Var], sub: Substitution ): Substitution = {
    Substitution( sub.map.filter( pair => !delvars.contains( pair._1 ) ) )
  }
  ...
  def createSubstFromListOfPairs( l: List[( FOLExpression, FOLExpression )] ): Substitution = {
    Substitution( l.map( x => ( x._1.asInstanceOf[Var], x._2 ) ) )
  }

  def matchSetOfTuples( moduloVarList: List[Var], s1: List[( FOLExpression, FOLExpression )], s2: List[( FOLExpression, FOLExpression )] ): Option[( List[( FOLExpression, FOLExpression )], List[( FOLExpression, FOLExpression )] )] = ( s1, s2 ) match {
    ...
    case ( ( FOLAtom( f1, args1 ), Atom( f2, args2 ) ) :: s, s2 ) if args1.length == args2.length && f1 == f2 => {
      return matchSetOfTuples( moduloVarList, args1.zip( args2 ) ::: s, s2 )
    }
    case _ => matchSetOfTuples1( moduloVarList, s1, s2 )
  }

  def matchSetOfTuples1( moduloVarList: List[Var], s1: List[( FOLExpression, FOLExpression )], s2: List[( FOLExpression, FOLExpression )] ): Option[( List[( FOLExpression, FOLExpression )], List[( FOLExpression, FOLExpression )] )] = ( s1, s2 ) match {
  ...
  def matchSetOfTuples2( moduloVarList: List[Var], s1: List[( FOLExpression, FOLExpression )], s2: List[( FOLExpression, FOLExpression )] ): Option[( List[( FOLExpression, FOLExpression )], List[( FOLExpression, FOLExpression )] )] = ( s1, s2 ) match {
  ...
  def matchSetOfTuples3( moduloVarList: List[Var], s1: List[( FOLExpression, FOLExpression )], s2: List[( FOLExpression, FOLExpression )] ): Option[( List[( FOLExpression, FOLExpression )], List[( FOLExpression, FOLExpression )] )] =
    ( s1, s2 ) match {
      case ( ( ( x: Var, v ) :: s ), s2 ) if !freeVariables( v ).contains( x ) && !moduloVarList.contains( x ) =>
        val lst1 = applySubToListOfPairs( s, restrictSubstitution( moduloVarList, Substitution( Substitution( x, v ).map ) ) )
        val lst2 = applySubToListOfPairs( s2, restrictSubstitution( moduloVarList, Substitution( Substitution( x, v ).map ) ) )
      ...
      case ( ( ( Const( name1 ), x: Var ) :: s ), s2 ) => None

      case ( ( ( y: Var, x: Var ) :: s ), s2 ) if x != y => None
