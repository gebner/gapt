  ...
  def convert( iproof: IvyResolutionProof, map: ProofMap ): ( RobinsonResolutionProof, ProofMap ) = {
    ...
    iproof match {
      ...
      case IPropositional( id, exp, clause, parent ) =>
        def remove_first( el: FormulaOccurrence, l: Seq[FormulaOccurrence] ): ( FormulaOccurrence, List[FormulaOccurrence] ) = l.toList match {
          case x :: Nil =>
        ...
        def remove_firsts( fs: Seq[FormulaOccurrence], l: Seq[FormulaOccurrence] ): ( List[FormulaOccurrence], Seq[FormulaOccurrence] ) = {
        ...
        def connect( ivy: List[FormulaOccurrence], robinson: Seq[FormulaOccurrence] ): List[FormulaOccurrence] = ivy match {
          case x :: xs =>
        ...
        def find_matching( what: List[FormulaOccurrence], where: List[FormulaOccurrence] ): List[FormulaOccurrence] = what match {
          case x :: xs =>
