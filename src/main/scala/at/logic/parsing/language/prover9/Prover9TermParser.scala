...
abstract trait Prover9TermParserA extends JavaTokenParsers with PackratParsers {
  ...
  *  (starting with u-z)*/
  ...
  lazy val allformula_ : PackratParser[FOLFormula] = ( "all" ~> variable ~ ( allformula_ | exformula_ | literal2 ) ) ^^ { case v ~ f => AllVar( v, f ) }
  lazy val exformula_ : PackratParser[FOLFormula] = ( "exists" ~> variable ~ ( allformula_ | exformula_ | literal2 ) ) ^^ { case v ~ f => ExVar( v, f ) }
  ...
  def orderings : Parser[FOLFormula] = term ~ """(<|>)=?""".r  ~ term ^^ { case t1 ~ sym ~ t2 => Atom(ConstantStringSymbol(sym), List(t1,t2))}
  */
  ...
  lazy val constant: PackratParser[Const] = conssymb ^^ {
    case x => FOLConst( x )
  }
  lazy val variable: PackratParser[Var] = varsymb ^^ { FOLVar( _ ) }
  ...
  val topformula = { Or( TopC, Neg( TopC ) ) }
  val bottomformula = { And( BottomC, Neg( BottomC ) ) }
  ...
  def normalizeFormula( f: FOLFormula ): FOLFormula = {
    val freevars: List[( Var, Int )] = freeVariables( f ).zipWithIndex
    val pairs: List[( Var, Var )] = freevars.map( x => { ( x._1, FOLVar( "v" + x._2 ) ) } )
