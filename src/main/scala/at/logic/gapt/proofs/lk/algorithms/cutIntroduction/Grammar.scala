...
class MultiGrammar( val us: Map[FOLFormula, List[List[FOLTerm]]], val ss: List[( List[FOLVar], List[List[FOLTerm]] )] ) {
  ...
  // compute termlistlist[ a / ss ]
  private def substAll( termlistlist: List[List[FOLTerm]], a: List[FOLVar], ss: List[List[FOLTerm]] ) =
    termlistlist.flatMap( termlist => ss.map( s => termlist.map( t => FOLSubstitution( a zip s )( t ) ) ) )

  // returns the MultiLanguage generated by this MultiGrammar
  def language = us.keys.map( formula => {
    val termlistlist = us( formula )
...
//            System.out.println("newpairs.length: " + newpairs.length)
//            System.out.println("newpairs: " + newpairs)
