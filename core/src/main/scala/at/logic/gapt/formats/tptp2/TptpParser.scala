...
class TptpParser( val input: ParserInput ) extends Parser {
  ...
  def annotated_formula = rule {
    atomic_word ~ "(" ~ Ws ~ name ~ Comma ~ formula_role ~ Comma ~ formula ~ annotations ~ ")." ~ Ws ~>
      ( TptpFormulaInput( _, _, _, _, _ ) )
  }
  def formula_role = rule { atomic_word }
  def annotations = rule { ( Comma ~ source ~ optional_info ~> ( ( s, o ) => Annotations( s, o ) ) ).? }

  def formula = rule { ( logic_formula ~> ( _.asInstanceOf[FOLFormula] ) ) | fof_sequent }
  def logic_formula: Rule1[HOLFormula] = rule { binary_formula | unitary_formula }
  def binary_formula = rule { binary_nonassoc | binary_assoc }
  def binary_nonassoc = rule { unitary_formula ~ binary_connective ~ unitary_formula ~> ( ( a, c, b ) => c( a, b ) ) }
  def binary_assoc = rule { or_formula | and_formula }
  def or_formula = rule { unitary_formula ~ ( "|" ~ Ws ~ unitary_formula ).+ ~> ( ( a, as ) => Or.leftAssociative( a +: as: _* ) ) }
  def and_formula = rule { unitary_formula ~ ( "&" ~ Ws ~ unitary_formula ).+ ~> ( ( a, as ) => And.leftAssociative( a +: as: _* ) ) }
  def unitary_formula: Rule1[HOLFormula] = rule { fof_quantified_formula | fof_unary_formula | atomic_formula | "(" ~ Ws ~ logic_formula ~ ")" ~ Ws }
  def fof_quantified_formula = rule { fol_quantifier ~ "[" ~ Ws ~ fof_variable_list ~ "]" ~ Ws ~ ":" ~ Ws ~ unitary_formula ~> ( ( q: QuantifierHelper, vs, m ) => q.Block( vs, m ) ) }
  def fof_variable_list = rule { variable.+.separatedBy( Comma ) }
  def fof_unary_formula = rule { ( unary_connective ~ unitary_formula ~> ( ( c, f ) => c( f ) ) ) | fol_infix_unary }
