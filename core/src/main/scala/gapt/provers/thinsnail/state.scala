...
class Cls(
  ...
  def lctx = proof.lctx
  def ass: Set[Int] = proof.assertions
  def assertion: Set[Int] = proof.assertions

  def clauseWithAssertions: ( Sequent[Term], Set[Int] ) =
    ( clause, assertion )

  val maximal: Vector[SequentIndex] = for {
    ( a, i ) <- clause.zipWithIndex.elements
    if !clause.elements.exists { x => a != x && state.termOrdering.lt( a, x ) }
  } yield i

  val selected: Vector[SequentIndex] =
    ( maximal.filter { _.isAnt } ++ clause.indicesSequent.antecedent ).take( 1 )

  val weight: Int = clause.elements.map( _.size ).sum
  ...
  override def toString: String =
    s"[$index] ${proof.clause.toString}   (max = ${maximal mkString ", "}) (sel = ${selected mkString ", "}) (w = $weight)"
...
class EscargotState( val ctx: MutableContext ) {
  val termCtx = new Ctx( ctx )
  val eqFnSym = termCtx.internFnSym( EqC.name, 2 )
